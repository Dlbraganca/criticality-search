#include"./headers/local_bfs.h"

local_bfs::local_bfs(const char* CK_PARAM_FILE) {

}

local_bfs::local_bfs(systemState SYSTEM, criticality& CRITICAL_DATA, std::string CK_PARAM_FILE) {
...
systemState local_bfs::get_best_heuristic(std::vector<systemState>& pile) {
...
void local_bfs::main() {
	...
	pile.push(systemState(newMeasuremet, critical_data, get_crit_type())); //estado inicial
		...
		else
		{
			for (int i = atualMeasurement.size() - 1; i >= 0; i--)
			{
				...
				else if (referenceVector[i] == 1)
				{
					...
					newMeasuremet[i] = 1;
					systemState newState(newMeasuremet, critical_data, get_crit_type(), nextState); //novo estado
