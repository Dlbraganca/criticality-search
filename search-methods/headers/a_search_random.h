...
class a_search_random : public ck_search
{
public:
	a_search_random(const char* CK_PARAM_FILE);
	a_search_random(systemState, criticality&, std::string);

	std::stack<std::vector<unsigned int>> get_result() { return result; }
private:
	double stoppingCriterion = 100; //n de interacoes maxima
	double metric = 100;
